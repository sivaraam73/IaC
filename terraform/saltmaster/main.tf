...
resource "null_resource" "run_ansible" {
  ...
  provisioner "local-exec" {
    command   = "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -u almalinux -v -i $(pwd)/playbooks/inventory -u almalinux --private-key $(pwd)/.ssh/<your-pte-key> $(pwd)/playbooks/install-saltmaster.yml" 
  }
